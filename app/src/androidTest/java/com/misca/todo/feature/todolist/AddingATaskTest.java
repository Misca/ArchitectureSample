...
public class AddingATaskTest {

    @Rule
    public ActivityTestRule<MainActivity> mActivityTestRule = new ActivityTestRule<>(MainActivity.class);

    @Test
    public void addingATaskTest() {
        ...
        ViewInteraction appCompatEditText = onView(
                allOf(childAtPosition(
                        ...
                        0),
                      isDisplayed()));
        appCompatEditText.perform(replaceText("abc test\n"), closeSoftKeyboard());

        pressBack();

        ViewInteraction appCompatEditText2 = onView(
                allOf(withText("abc test\n"),
                      childAtPosition(
                              childAtPosition(
                                      withId(R.id.main_container),
                                      1),
                              0),
                      isDisplayed()));
        appCompatEditText2.perform(replaceText("abc test\n\n"));
